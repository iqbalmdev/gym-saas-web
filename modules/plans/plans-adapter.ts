...
import { endpoints } from '@/modules/plans/plans-endpoints';
...
import type {
    ...
    PlansWriter,
    UpdatePlanInput,
} from '@/modules/plans/plans-ports';
...
function normalizePlan(raw: z.infer<typeof planSchema>, gymOrgId: string): MembershipPlan {
    return {
        id: raw.id,
...
function listQuery(input: { kind?: PlanKind; active?: boolean; limit?: number; offset?: number }): string {
...
export function createPlansAdapter(http: HttpClient): PlansReader & PlansWriter {
    return {
        async list({ accessToken, gymOrgId, kind, active, limit, offset }) {
            ...
            return {
                plans: {
                    ...parsed.plans,
                    items: parsed.plans.items.map((item) => normalizePlan(item, gymOrgId)),
                },
            };
        },

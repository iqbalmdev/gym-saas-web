...
const STAFF_INVITE_ERROR_COPY: Record<string, string> = {
    STAFF_INVITE_FORBIDDEN: 'You cannot manage staff invites for this gym with this account.',
    ...
    STAFF_INVITE_INVALID_TRANSITION: 'That invite cannot be changed from its current status.',
...
export function staffInviteErrorMessage(code: string, fallbackMessage?: string): string {
    return STAFF_INVITE_ERROR_COPY[code] ?? fallbackMessage ?? 'Something went wrong. Please try again.';
}

...
import { membershipPaymentStatusLabel } from "@/lib/display/membership-invite-labels";
import { updateSubscriptionPaymentAction } from "@/lib/features/subscriptions/actions";
import type {
  RenewalDueItem,
...
const PAYMENT_OPTIONS: SubscriptionPaymentStatus[] = [
  "unpaid",
  "paid",
  "partial",
];

export function RenewalsAdminPanel({
  gymName,
  renewals,
  windowLabel,
  listError,
}: RenewalsAdminPanelProps) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-[var(--color-fg-muted)]">
          Subscriptions ending {windowLabel}. Payment badges nudge — they do
          not auto lock check-in.
        </p>
      </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {PAYMENT_OPTIONS.map((status) => (
                    <Button
                      ...
                      onClick={() =>
                        handlePaymentUpdate(item.id, status, item.priceAmount)
                      }
                    >

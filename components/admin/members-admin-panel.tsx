...
} from "@/lib/display/membership-invite-labels";
import {
  createMembershipInviteAction,
  revokeMembershipInviteAction,
} from "@/lib/features/membership-invites/actions";
import type {
  MembershipInvite,
...
const PAYMENT_OPTIONS: MembershipPaymentStatus[] = [
  "unpaid",
  "paid",
  "partial",
];
...
export function MembersAdminPanel({
  ...
  const [basePaymentStatus, setBasePaymentStatus] =
    useState<MembershipPaymentStatus>("unpaid");
  const [addonPlanId, setAddonPlanId] = useState("");
  const [addonPaymentStatus, setAddonPaymentStatus] =
    useState<MembershipPaymentStatus>("unpaid");
  ...
  function planName(planId: string): string {
    const plan =
      basePlans.find((item) => item.id === planId) ??
      addonPlans.find((item) => item.id === planId);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-[var(--color-fg-muted)]">
          Invite clients by email with a Base plan (optional Trainer add-on).
          Payment badges are informational — entitlement follows subscription
          dates after accept.
        </p>
      </div>
      ...
      <section className="rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-panel)] md:p-6">
        ...
        {basePlans.length === 0 ? (
          <p className="mt-3 text-sm text-[var(--color-fg-muted)]">
            Create an active Base plan under Plans before sending a membership
            invite.
          </p>
        ) : (

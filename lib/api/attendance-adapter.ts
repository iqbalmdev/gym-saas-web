...
function normalizeAttendance(
  raw: z.infer<typeof attendanceSchema>,
  gymOrgId: string,
): Attendance {
  return {
    id: raw.id,
...
export function createAttendanceAdapter(
  http: HttpClient,
): AttendanceReader & AttendanceWriter {
  return {
    async listForDay({ accessToken, gymOrgId, day, limit, offset }) {
      ...
      return {
        attendances: {
          ...parsed.attendances,
          items: parsed.attendances.items.map((item) =>
            normalizeAttendance(item, gymOrgId),
          ),
        },
      };
    },

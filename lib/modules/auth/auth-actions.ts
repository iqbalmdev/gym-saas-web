...
import type { CreateGymOrgInput } from "@/lib/modules/gym-orgs/gym-orgs-ports";
...
async function redirectAfterAuth(snapshot: SessionSnapshot): Promise<never> {
  ...
  redirect(
    resolvePostAuthPath({ lane: snapshot.lane, gymOrgCount }),
  );
}
...
export async function requestStaffOtpAction(input: {
  email: string;
}): Promise<AuthActionResult> {

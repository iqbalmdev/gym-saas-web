...
import { endpoints } from '@/lib/modules/membership-invites/membership-invites-endpoints';
...
function normalizeInvite(raw: z.infer<typeof inviteSchema>, gymOrgId: string): MembershipInvite {
    return {
        id: raw.id,
...
function normalizeDataGrants(raw: z.infer<typeof dataGrantsSchema>): MyDataGrants {
    return {
...
export function createMembershipInvitesAdapter(http: HttpClient): MembershipInvitesReader & MembershipInvitesWriter {
    return {
        ...
        async list({ accessToken, gymOrgId, limit, offset }) {
            ...
            return {
                membershipInvites: {
                    ...parsed.membershipInvites,
                    items: parsed.membershipInvites.items.map((item) => normalizeInvite(item, gymOrgId)),
                },
            };
        },
        ...
        async accept({ accessToken, membershipInviteId, body }) {
            ...
            return {
                membershipInvite: normalizeInvite(parsed.membershipInvite, parsed.membershipInvite.gymOrgId ?? ''),
        ...
        async revoke({ accessToken, gymOrgId, membershipInviteId }) {
            const raw = await http.request<unknown>({
                path: endpoints.gymOrgMembershipInviteRevoke(gymOrgId, membershipInviteId),
                method: 'POST',
                accessToken,
            });
            const parsed = inviteEnvelopeSchema.parse(raw);

import type { PlansReader, PlansWriter, CreatePlanInput, UpdatePlanInput, PlanKind } from "@/lib/modules/plans/plans-ports";
...
export function createCreatePlan(deps: { plans: PlansWriter }) {
  return async function createPlan(input: {
    accessToken: string;
    gymOrgId: string;
...
export function createSoftDeletePlan(deps: { plans: PlansWriter }) {
  return async function softDeletePlan(input: {
    accessToken: string;
    gymOrgId: string;
    planId: string;
  }) {

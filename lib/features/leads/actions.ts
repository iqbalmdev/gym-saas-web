...
import { leadErrorMessage, leadWarningMessage } from "@/lib/display/lead-errors";
import type { LeadStatus } from "@/lib/ports/leads";
...
async function requireStaffAdminGym(): Promise<
  ...
  | { ok: false; result: LeadActionResult }
> {
...
export async function createLeadAction(input: {
  ...
  try {
    ...
    return {
      ok: true,
      warning: warning
        ? leadWarningMessage(warning.code, warning.message)
        : undefined,
    };
  } catch (error) {
    return fail(error);

...
test.describe('Staff invites', () => {
    test('Admin can open Settings and see create + list', async ({ staffAdmin, settingsPage, page }) => {
    ...
    test('Admin can create a staff invite', async ({ staffAdmin, settingsPage, page }) => {
    ...
    test('Staff without gym sees inbox and can accept', async ({ staffNoGym, page }) => {
        await expect(page).toHaveURL(/\/admin\/settings/);

...
| Postman API collection | Sibling at **`5c3ee88`** (Nutrition search `units[].id`; Attendance check-out/present; `GET /me/gym`; Convert Lead wired on main). Pulled 2026-09-11. Cloud inject still blocked — Postman MCP 401; Desktop Import the sibling JSON |
...
| Feature modules | M1 auth; M2 Settings-first + invites; **M3 membership invites + my-data-grants**; **M4 plans/addons**; **roster / attendance / renewals** incl. **trainer assignment**; **profile & progress**; **M6/M7 coaching (client + staff diet/workout templates, assign, schedule upsert)**; **M9 nutrition**; **M10 health sync**; **M11 leads** incl. **convert**. Destructive actions across every module are behind one confirm step |
...
**Summary:** Roster, attendance desk, renewals inbox, CRM convert-lead, coaching (client diet/workouts + staff templates/assign/schedule), nutrition diary + catalog logging, and client my-data-grants wired against Postman tip `5c3ee88` (sibling pulled 2026-09-11; cloud collection still waiting on Postman MCP re-auth). Several attendance/subscription endpoints are still unwired. Agent OS repaired after the `e537810` rule drift; tooling + CI landed (ADR-0006). Domain slices live at top-level `modules/` (ADR-0008 amending ADR-0007). Admin shell rebuilt on Base UI's `Sidebar` primitive. Renewals, CRM and the attendance desk share a reusable queue + rail layout; the app displays as **Yeah Buddy**.
...
7f. ~~Health Sync~~ — **Done.** `/client/health` lists wearable connections (Connect / Disconnect) and synced daily metrics; member detail gains a WEARABLES-gated staff panel. `POST /me/wearable-metrics/sync` is deliberately unwired — the metric push is a device read (PRD §M10, flows F10.1–F10.2), and a web form for it would mean typing in your own step count. `GrantAware<T>` moved to `lib/domain/`. See `docs/progress/2026-08-29-health-sync.md`.
7g. ~~Nutrition~~ — **Done.** `/client/nutrition` diary (read + log/unlog extras) and searchable catalog with serving ids from Postman `5c3ee88`; CALORIES-gated staff diary on member detail. See `docs/progress/2026-08-29-nutrition.md` + `docs/progress/2026-09-11-nutrition-log-extra.md`.
7h. ~~Coaching~~ — **Done.** Client `/client/diet` + `/client/workouts` (my plan, schedule, streak, complete/uncomplete). Admin `/admin/coaching` diet + workout template CRUD/duplicate; member detail staff diet assign, workout schedule upsert, week read + streak (`WORKOUT_PLANS` grant-aware). `GET /exercises/search` on the port for future template builder UI — MVP templates use seeded exercise ids. See `docs/progress/2026-09-13-admin-coaching.md` + `e2e/coaching.spec.ts`.

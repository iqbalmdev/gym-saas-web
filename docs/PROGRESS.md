...
| Postman API collection | Sibling at **`9b0b561`** (Convert Lead + trainer list + Nutrition/Coaching/Health Sync). Re-pulled 2026-08-29 — no upstream change. Cloud inject still blocked — Postman MCP 401 even after `mcp_auth`; Desktop Import the sibling JSON |
...
| Feature modules | M1 auth; M2 Settings-first + invites; **M3 membership invites + my-data-grants**; **M4 plans/addons**; **roster / attendance / renewals**; **profile & progress**; **M9 nutrition (read + unlog)**; **M10 health sync**; **M11 leads** |
...
7g. ~~Nutrition~~ — **Done, minus logging.** `/client/nutrition` shows the day diary (totals + five meal slots, Remove on extras) and a searchable food catalog; member detail gains a CALORIES-gated staff diary panel. **`POST /me/calorie-logs/items` is unbuildable from the current contract** — it requires a `servingId`, and `GET /foods/search` omits serving ids by design, so there is no path from a search hit to a log. Needs a backend change before the web can log food. See `docs/progress/2026-08-29-nutrition.md`.
